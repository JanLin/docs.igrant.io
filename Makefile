...
CANDIDATE ?= "dev"
CONTAINER_USER_DOCS ?= "igrant_user_docs"
...
run: ## Run user docs locally for development purposes
	docker run \
		$(CONTAINER_DEFAULT_RUN_FLAGS) \
		--expose 5000 \
		-e VIRTUAL_HOST=docs.$(PROJECT).dev \
...
deploy/production: $(DEPLOY_VERSION_FILE) ## Deploy to K8s cluster (e.g. make deploy/{preview,staging,production})
	kubectl set image deployment/igrant-user-docs igrant-user-docs=$(DEPLOY_VERSION) -n production

deploy/staging: $(DEPLOY_VERSION_FILE) ## Deploy to K8s cluster (e.g. make deploy/{preview,staging,staging})
	kubectl set image deployment/igrant-user-docs igrant-user-docs=$(DEPLOY_VERSION) -n staging
